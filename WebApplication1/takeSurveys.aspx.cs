...
namespace WebApplication1
{
    public partial class takeSurveys : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (objReader.Read())
                {
                    ...
                    TakeSurveyButton1.Text = String.Format("{0}", objReader["title"]);
                }
                else
                {
                ...
                if (objReader.Read())
                {
                    //lblResults.Text = String.Format("Success, ID={0}", objReader["ID"]);
                    //int nID = Convert.ToInt32(objReader["ID"]);
                    TakeSurveyButton2.Text = String.Format("{0}", objReader["title"]);
                }
                else
                {
                    // lblResults.Text = "Login Failed";
                ...
                if (objReader.Read())
                {
                    //lblResults.Text = String.Format("Success, ID={0}", objReader["ID"]);
                    //int nID = Convert.ToInt32(objReader["ID"]);
                    TakeSurveyButton3.Text = String.Format("{0}", objReader["title"]);
                }
                else
                {
                    // lblResults.Text = "Login Failed";
        ...
        protected void TakeSurveyButton1_Click(object sender, EventArgs e)
        {
            Response.Redirect("currentSurvey.aspx?SurveyID=1");
        }
        protected void TakeSurveyButton2_Click(object sender, EventArgs e)
        {
            Response.Redirect("currentSurvey.aspx?SurveyID=2");
        }
        protected void TakeSurveyButton3_Click(object sender, EventArgs e)
        {
            Response.Redirect("currentSurvey.aspx?SurveyID=3");
        }
        protected void TakeSurveyButton4_Click(object sender, EventArgs e)
        {
            Response.Redirect("currentSurvey.aspx?SurveyID=4");
        }
        protected void TakeSurveyButton5_Click(object sender, EventArgs e)
        {
            Response.Redirect("currentSurvey.aspx?SurveyID=5");
        }
}
}